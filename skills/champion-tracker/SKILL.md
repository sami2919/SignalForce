...
A champion is anyone who has shown genuine interest in Collinear AI:
...
2. **Evaluate the new company** — run `prospect-researcher` skill on the new company. Score against ICP criteria from `.agents/gtm-context.md`.
...
3. **Route by ICP score:**
   ...
   - **ICP grade C or D**: Log the move in HubSpot. Add a note with new company. Set a 90-day follow-up task — re-evaluate if their RL signals grow.
...
Reference `templates/email-sequences/champion-job-change.md` for the full template.
...
- **Reference shared history** — name the specific interaction ("when we spoke about your reward modeling setup at [OldCo]").
