"""GitHub RL Repository Scanner.

Detects organizations actively investing in reinforcement learning by monitoring
their GitHub repositories. Scans for RL-related topics and libraries, filters to
...
class GitHubRLScanner:
    """Scanner that detects orgs investing in reinforcement learning via GitHub."""
    ...
    def __init__(self, config: AppConfig | None = None) -> None:
        ...
        self._client = GitHubClient(token=self._config.github_token)
    ...
    def scan(self, lookback_days: int = 7) -> ScanResult:
        """Run a full GitHub RL scan.

        Steps:
        1. Build search queries for RL topics within the lookback window.
        ...
        Args:
            lookback_days: How many days back to scan for new/updated repos.
        ...
        for query in queries:
            try:
                response = self._client.search_repos(query)
            ...
            for repo in items:
                ...
                if not self._is_organization(owner):
                    continue
                org_name: str = owner["login"]
                # Deduplicate repos by id within org accumulator
                existing_ids = {r["id"] for r in org_repos[org_name]}
                if repo["id"] not in existing_ids:
                    org_repos[org_name].append(repo)
                    total_raw += 1

        # Score each org and build signals; deduplicate by keeping highest score
        # (We already accumulate all repos across queries before scoring, so a
        # single score is produced per org naturally.)
        signals: list[Signal] = []
        for org_name, repos in org_repos.items():
            repo_count = len(repos)
            # Use forks_count as a proxy for contributor engagement
            contributor_count = sum(r.get("forks_count", 0) for r in repos)
            score = self._score_org(repo_count, contributor_count)
            signal = self._create_signal(org_name, repos, score)
    ...
    def _build_search_queries(self, lookback_days: int) -> list[str]:
        """Build GitHub search query strings covering RL topics and libraries.

        Each query uses the `topic:` qualifier combined with a `pushed:>` date
        filter so we only surface recently active repositories.
        ...
        # One query per RL topic
        for topic in self.RL_TOPICS:
            queries.append(f"topic:{topic} pushed:>{since_date}")

        # One query per RL library (uses the "topic:" tag or repo name keyword)
        for lib in self.RL_LIBRARIES:
            queries.append(f"topic:{lib} pushed:>{since_date}")

        return queries

    def _is_organization(self, owner: dict) -> bool:
        """Return True if the repo owner is a GitHub Organisation."""
        return owner.get("type") == "Organization"

    def _score_org(self, repo_count: int, contributor_count: int) -> SignalStrength:
        """Score an organisation's RL investment level.
        ...
        Args:
            repo_count: Number of RL-related repos found for the org.
    ...
    def _create_signal(
        ...
        """Build a Signal object for a detected RL-active organisation.
        ...
        Returns:
            Signal with GITHUB_RL_REPO type and repo_name in metadata.
        """
        # Use the most recently pushed repo as the canonical source
        sorted_repos = sorted(
            repos,
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Scan GitHub for organisations actively investing in RL.",
...
def main(argv: list[str] | None = None) -> None:
    """CLI entry point for the GitHub RL scanner."""
    parser = _build_arg_parser()
    args = parser.parse_args(argv)

    scanner = GitHubRLScanner()
