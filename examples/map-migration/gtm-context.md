# GTM Context: Lumera (Marketo Migration ICP)

> **Note:** Lumera is a fictional warehouse-native MAP vendor used to demonstrate
> this example ICP. Swap in your own company details to activate it.
...
- **Warehouse data gap** — Marketo/HubSpot require reverse ETL (Hightouch, Census) to
  activate product usage signals; Lumera joins natively at send time
- **Custom object constraints** — Legacy MAPs limit trigger flexibility; Lumera exposes
  the full Salesforce/warehouse object graph
- **Salesforce sync failures** — HubSpot's bidirectional sync breaks at scale; Lumera
  is Salesforce-native
- **Personalization ceiling** — Static CRM data + slow builders; Lumera embeds AI agents
  with real-time warehouse traits per touch

**Lumera campaign anatomy:**
...
## Example Customer Profiles (Signal Calibration)

Fictional customer profiles that match the ICP tiers. Use them to calibrate signal
accuracy — if your config would have surfaced them, the signals are correct.

| Customer | What they replaced | Key signal | Tier |
|----------|--------------------|------------|------|
| DataStack Co | Marketo + Pardot + 3 scoring tools | Reverse ETL (Hightouch) + MOPs hire | Tier 1 |
| CivicWorks | 5 tools, went live in 6 weeks | MAP consolidation initiative + data warehouse | Tier 3 |
| StayHost | Pardot, 200K+ contacts | Large contact DB + Pardot pain | Tier 3 |
| FunnelMetrics | Product analytics (Mixpanel-adjacent) + HubSpot | PLG + MAP event gap | Tier 5 (PLG) |
| OutboundIQ | GTM-native, warehouse-first from the start | Snowflake + Segment + growth engineer hire | Tier 5 (PLG) |
| SignalWarm | Series A, PLG motion + lifecycle gap | Product analytics + reverse ETL need | Tier 5 (PLG) |

**Pattern:** The strongest-fit profiles have a PLG motion + a warehouse + a MAP that can't consume
product events natively. The PLG tier (Tier 5 in config) is the highest-signal pattern.
...
- Reference their Salesforce stack — Lumera's sync is a concrete differentiator
